...
def assert_setup_payloads():
    ...
    bot_payload = build_launch_payload(
        build_setup_state(
            player_bot_flags=[False, True],
            turn_mode=MANUAL_TURN_MODE,
        )
    )
    assert bot_payload[4] == SEQUENTIAL_TURN_MODE

    solo_payload = build_launch_payload(build_setup_state(mode_variant="solo_bot", turn_mode=MANUAL_TURN_MODE))
    assert len(solo_payload[0]) == 2
    assert solo_payload[0][1].is_bot
    assert solo_payload[5]["mode_variant"] == "solo_bot"
    ...
    assert apply_rules_hotkey(rules_state, pygame.K_4)
    assert rules_state["mode_variant"] == "best_of_three"
    assert apply_rules_hotkey(rules_state, pygame.K_z)
    assert rules_state["ai_level"] == "easy"


def assert_custom_mode_helpers():
    mode_data = {
        "name": "Thu thach rieng",
        "player_names": ["Lan", "Bot do"],
        "ai_level": "smart",
        "layout_id": "tower",
    ...
    assert len(players) == 2 and players[1].is_bot
    ...
    assert apply_custom_turn_hotkey(turn_state, pygame.K_c)
    assert turn_state["ai_level"] == "smart"
    ...
    assert apply_custom_variant_hotkey(variant_state, pygame.K_4)
    assert variant_state["mode_variant"] == "solo_bot"
    preset_samples = [
    ...
    assert len(filter_custom_mode_presets(preset_samples, "human")) == 2
    assert len(filter_custom_mode_presets(preset_samples, "bot")) == 1
    ...
    assert launch_payload[0][1].is_bot
    assert launch_payload[4] == SEQUENTIAL_TURN_MODE
...
def assert_filter_helpers():
    ...
    assert len(filter_history_entries(sample_history, "bot")) == 2
...
def render_smoke_surface():
    ...
    draw_menu_option(
        ...
        "Choi mac dinh",
        "Vao game nhanh voi preset tran, layout moi va bot AI.",
        (247, 223, 184),
        PALETTE["gold_dark"],
        hovered=True,
        hotkey_label="1",
    ...
    draw_choice_card(
        ...
        "Solo vs Bot",
        "Tu dong tao 1 nguoi va 1 bot, phu hop de test nhanh.",
        active=True,
    )
    ...
    players = [Player("Nguoi 1"), Player("AI Doi thu", is_bot=True), Player("Nguoi 3")]
    players[0].score = 26
    players[1].score = 18
    players[2].score = 12
    ...
    draw_result_meta_block(surface, tiny_font, tiny_font, pygame.Rect(540, 58, 650, 86), session_stub, {"wins": {"Nguoi 1": 2, "AI Doi thu": 1}})
